...
function itr_create_games(lobby_details) {
    ...
    for (let i = 0; i < lobby_details.games.length; i++) {
        game_grid_payload += "<div class=\"block text-center\" id=\"" + lobby_details.games[i].game_slug + "\"></div>";
    }
    ...
    if (session_user.is_host) {
        if (lobby_details.in_progress) {
            host_action_payload = "<div class=\"flex justify-center mt-4 mb-6\">\n" +
                "    <button type=\"button\" class=\"inline-flex items-center px-2 py-1 rounded-md shadow-sm text-lg font-medium text-white border border-base-200 bg-gradient-to-r from-yellow-500 to-yellow-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500\" onclick=\"reset_lobby()\">\n" +
    ...
    document.getElementById("itr_ele_groups").innerHTML = "<div class=\"grid " + grid_break + " gap-x-6 gap-y-6 p-4\">" +
        ...
        "</div>" +
        host_action_payload;
    // Loop through each game again and update
    for (let i = 0; i < lobby_details.games.length; i++) {
        itr_update_game(lobby_details.games[i]);
    }
}
...
function itr_update_game(game_details) {
    ...
    for (let j = 0; j < game_details.players.length; j++) {
        ...
        let turns = game_details.turn_seat_pos === game_details.players[j].seat_pos ? game_details.turns_remain : 0;
        // Check for dead filter
        let filter = game_details.players[j].is_dead ? "filter grayscale" : "";
        player_payload += "<div class=\"block text-center w-28\">\n" +
            "    <h1 class=\"text-base-content font-medium text-sm\">\n" +
            "        " + game_details.players[j].nickname + " " + create_stat_dot(game_details.players[j].sockets_open, "", "itr_stat_player_dot_" + game_details.players[j]._id) + "\n" +
            "    </h1>\n" +
            "    <div class=\"flex flex-col items-center -space-y-3\">\n" +
            "        <img class=\"h-12 w-12 rounded-full " + filter + "\" src=\"/public/avatars/" + game_details.players[j].avatar + "\" alt=\"\">\n" +
            card_icon(game_details, j, turns) +
            "    </div>\n" +
            "</div>";
    }
    document.getElementById(game_details.game_slug).innerHTML = "<div class=\"block text-center\">\n" +
        "    <h1 class=\"text-base-content font-medium text-sm\">\n" +
        "        Game Room: " + game_details.game_slug + " <span class=\"animate-pulse inline-flex rounded-full h-1.5 w-1.5 mb-0.5 align-middle bg-" + (game_details.in_progress ? "success" : "info") + "\"></span>\n" +
        "    </h1>\n" +
        "    <h1 class=\"text-base-content font-medium text-sm\">\n" +
        ...
        "        </svg>\n" +
        "        " + game_details.players.length + "/" + game_details.room_size + "\n" +
        ...
        "        </svg>\n" +
        "        " + game_details.cards_remain + "/" + game_details.cards_total + " Cards\n" +
        ...
        "        </svg>\n" +
        "        " + game_details.ec_remain + " EC\n" +
        "    </h1>\n" +
        "    <div class=\"px-2 pb-2\">\n" +
        "        <progress class=\"progress " + (game_details.is_completed ? "progress-success" : "progress-info") + "\" value=\"" + (game_details.is_completed ? 100 : Math.ceil((((game_details.cards_total - (game_details.players.length * 5 + (6 - game_details.players.length))) - game_details.cards_remain) /game_details.cards_total) * 100)) + "\" max=\"100\"></progress>\n" +
        "    </div>\n" +
