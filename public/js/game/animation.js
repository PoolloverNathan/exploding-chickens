/*\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
Filename : exploding-chickens/public/js/game/animation.js
...
// Name : frontend-game.anm_draw_card(payload)
// Desc : trigger the draw card animation
function anm_draw_card(payload) {
    const temp = document.getElementById("anim_draw");
    const target = document.getElementById(payload.callback.card._id);
    // Handle lag from data update
    if (get_position(target).x === 0 && get_position(target).y === 0) {
        return;
    }
    // Add new animation
    new AnimationFrames({
        ...
        onstart () {
            temp.style.display = '';
            temp.style.backgroundImage = 'url(' + card_url(payload.callback.card) + ')';
...
// Name : frontend-game.anm_play_card(payload)
// Desc : trigger the play card animation
function anm_play_card(payload) {
    const temp = document.getElementById("anim_draw");
    const target = document.getElementById(payload.callback.card._id);
